...
- `outputs/tables/`: processed tables and aggregate summaries used by the text
  and figures.
- `scripts/`: experiment, analysis, and figure-generation scripts.
- `src/`: project source code used by the scripts.
- `configs/` and `config*.yaml`: configuration files for benchmark and model
  runs.
- `data/results*/`: saved generated-result artifacts used for reported
  evaluations and figure reconstruction.
- `requirements.txt` and `requirements-lightning.txt`: environment notes.
- `JSS_CLAIM_ARTIFACT_AUDIT.md`: the claim-to-artifact audit that maps reported
  manuscript claims to saved local evidence.
...
- Raw local execution logs are not included in the reviewer ZIP; the local
  repository retains them, while the reviewer package includes structured
  result artifacts, processed tables, figures, scripts, configs, and audit
  reports used by the manuscript.

All model checkpoints used by the experiments are public pretrained models and
can be reloaded by the scripts/configuration files in a compatible Python
environment.
...
The final checked build had no LaTeX errors, no unresolved citations, no
unresolved references, and no overfull boxes.
