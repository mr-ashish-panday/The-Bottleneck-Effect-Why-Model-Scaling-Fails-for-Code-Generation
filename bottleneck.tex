...
\midrule
HumanEval main & 164 & 100 & core comparison / pass@k \\
...
\begin{table*}[t]
\centering
\caption{Model Performance on HumanEval (100 samples per problem, 16,300-16,400 total attempts)}
...
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Checkpoint} & \textbf{HumanEval success \%} & \textbf{MBPP success \%} \\
\midrule
CodeGen-NL & 30.60 & 0.02 \\
CodeGen-Multi & 28.46 & 1.23 \\
CodeGen-Mono & 37.76 & 2.76 \\
\bottomrule
\end{tabular}
}
\end{table}

\textbf{Finding}: Mono is strongest on both benchmarks, but the ladder behaves differently across them. On HumanEval, Mono significantly exceeds both NL (37.76\% vs.\ 30.60\%, $p=0.0001$) and Multi (37.76\% vs.\ 28.46\%, $p=0.0001$), while Multi trails NL slightly (28.46\% vs.\ 30.60\%, $p=0.0443$). On MBPP, the ladder becomes cleanly monotone: NL reaches 0.02\%, Multi 1.23\%, and Mono 2.76\%, with significant improvements from NL to Multi ($p=0.0001$) and Multi to Mono ($p=0.0157$).

\textbf{Interpretation}: This is the strongest evidence in the paper against a pure family-comparison objection. Even within a single 350M architecture family, additional code- and Python-specific continued pretraining changes performance substantially. The HumanEval ladder is mixed because Multi underperforms NL, so we do not treat the ladder as a matched-compute law. The MBPP result is cleaner and supports the narrower claim that continued-pretraining plus curriculum effects materially improve small-model code generation within a single family.
...
An exploratory reduced ablation sweep across the ladder also suggests that the signature layer shifts with continued pretraining: NL's strongest surviving layer is 11, Multi's is 7, and Mono's is 13. We treat these layer assignments cautiously because the ladder sweep is smaller than the main HumanEval ablation study, but they are consistent with the broader claim that continued pretraining changes not only accuracy but also where executable-code computation is supported.
...
\textbf{4. Graded interventions remain limited}: We now include scaled-ablation pilots for GPT-2 Medium layer 12 and CodeGen layer 13, but both cover only one layer at a time on 10-problem / 5-sample subsets. Broader graded sweeps remain necessary before claiming a general law of thresholded behavior.
