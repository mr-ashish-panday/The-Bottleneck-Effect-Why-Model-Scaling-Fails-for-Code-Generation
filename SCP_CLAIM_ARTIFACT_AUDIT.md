# SCP Claim-to-Artifact Audit

Checks passed: 32
Checks failed: 0

This audit records the evidence gate for the Science of Computer Programming transfer package. The checks verify that manuscript-facing figures, benchmark summaries, syntax profiles, and robustness-control values are backed by local artifacts.
...
| PASS | Manuscript retargeted to SCP | `bottleneck.tex` uses `\journal{Science of Computer Programming}` |
| PASS | SCP-facing portal metadata exists | `SCP_PORTAL_METADATA.md` |

All audit checks recorded here passed for the current evidence package.
